...
def run_training():
    ...
    # poems_vector, word_to_int, vocabularies = process_poems2(os.path.join(curr_dir, 'tangshi.txt'))
    poems_vector, word_to_int, vocabularies = process_poems1(os.path.join(curr_dir, 'poems.txt'))
    # 生成batch
    ...
    for epoch in range(30):
        ...
        for batch in range(n_chunk):
            ...
            if batch % 20 ==0:
                torch.save(rnn_model.state_dict(), os.path.join(curr_dir, 'poem_generator_rnn'))
...
def gen_poem(begin_word):
    curr_dir = os.path.dirname(os.path.abspath(__file__))
    # poems_vector, word_int_map, vocabularies = process_poems2(os.path.join(curr_dir, 'tangshi.txt'))  #  use the other dataset to train the network
    poems_vector, word_int_map, vocabularies = process_poems1(os.path.join(curr_dir, 'poems.txt'))
    ...
    rnn_model.load_state_dict(torch.load(os.path.join(curr_dir, 'poem_generator_rnn')))
